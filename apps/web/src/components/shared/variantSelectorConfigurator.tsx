...
import { buildConfiguratorIntroHint, computeOptionState, filterOptionsForUpstreamSelection, gradeOptionState, type Dimension } from "./variantSelectorDimensions";
...
export function Configurator({ dimensions, variants, currentSelection, onPick, realignmentNotice = null, realignmentDimensionKey = null }: ConfiguratorProps) {
	...
	const gradeScopedVariants = activeGradeSlug ? variantsForGrade(variants, activeGradeSlug) : variants;
		...
		<div className="overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-ink-200)]" role="tablist" aria-label={dimension.label}>
			<div className="-ml-px -mt-px flex flex-wrap md:m-0 md:flex-nowrap md:divide-x md:divide-[var(--color-ink-200)]">
				{options.map((option) => {
					...
					const state = dimension.isGrade ? gradeOptionState(option.key, variants, currentSelection) : computeOptionState(dimension.key, option.key, variants, currentSelection);
					...
					return (
						<button
							...
							data-state={state}
							title={isOutOfStock ? "Out of stock for this grade" : undefined}
							className={classNames(
