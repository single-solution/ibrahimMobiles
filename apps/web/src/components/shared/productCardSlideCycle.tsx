...
import { getProductGradesInDisplayOrder, getVariantsInDisplayOrder, scopeProductToGrade } from "@/lib/productSummary";
...
export function ProductCardMediaCycle({
	...
	priority?: boolean;
	/** Product-level deal hint — not tied to the cycling grade / variant slide. */
	offerCount?: number;
}) {
	const activeSlide = slides[activeIndex] ?? slides[0];

	function renderDealAvailableBadge() {
		if (offerCount <= 0) {
			return null;
		}
		return <ProductDealAvailableBadge offerCount={offerCount} />;
	}
	...
	return (
		<>
          ...
          listing), just render the one badge. */}
			{offerCount > 0 ? <div className="absolute left-1.5 top-1.5 z-10 md:left-3 md:top-3">{renderDealAvailableBadge()}</div> : null}

			{pinnedGradeSlug ? (
				<div className="absolute right-1.5 top-1.5 z-10 md:right-3 md:top-3">
					<GradeBadge categorySlug={categorySlug} gradeSlug={pinnedGradeSlug} size="sm" />
				</div>
						...
						{slides.map((slide, index) =>
							slide.gradeSlug ? (
								<div key={slide.slideKey} className={`card-fade-stack__layer ${index === activeIndex ? "card-fade-stack__layer--active" : ""}`} aria-hidden={index !== activeIndex}>
									<GradeBadge categorySlug={categorySlug} gradeSlug={slide.gradeSlug} size="sm" />
								</div>
							) : null,
						)}
					</div>
				...
				{activeSlide
					? (() => {
							const labels = flattenChipGroups(activeSlide.titleChipGroups)
								.map((chip) => chip.label)
								.join(", ");
							return activeSlide.gradeSlug ? `${activeSlide.gradeSlug}: ${labels}` : labels;
						})()
					: null}
			</span>
		</>
...
export function useSlideCycle(slideCount: number, cycleKey: string, enabled: boolean) {
	...
	const staggerMs = useMemo(() => hashCycleOffset(cycleKey, GRADE_CYCLE_MS), [cycleKey]);
