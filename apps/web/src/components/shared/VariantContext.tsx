	...
	const pick = useCallback(
		(dimensionKey: string, optionKey: string): PickResult => {
			...
			if (dimensionKey === GRADE_DIMENSION_KEY) {
				const merged = mergeSelectionForGradeChange(before, optionKey, product.variants);
				const gradeScoped = variantsForGrade(product.variants, optionKey);
				const { selection } = resolvePickerSelection(gradeScoped, merged, GRADE_DIMENSION_KEY);
			...
			const scopedVariants = activeGrade ? variantsForGrade(product.variants, activeGrade) : product.variants;
			const { selection } = resolvePickerSelection(scopedVariants, proposed, dimensionKey);
			replace(selectionToUrlPatch(selection, attributeSlugs));
			return { clickedDimensionKey: dimensionKey, before, after: selection };
		},
