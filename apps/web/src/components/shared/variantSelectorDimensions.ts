...
import { attributeValuesOnVariant, findVariantBySelection, GRADE_DIMENSION_KEY, variantsForGrade } from "@/lib/catalog/pdpSelection";
...
export function buildDimensions(product: Product, attributeDefinitions: AttributeDescriptor[], grades: GradeDescriptor[], productConfig?: ProductAttributeConfig): Dimension[] {
...
function collectGradeOptions(variants: Variant[], grades: GradeDescriptor[]): DimensionOption[] {
...
export function filterOptionsForUpstreamSelection(
	...
	return dimension.options.filter((option) =>
		scoped.some((variant) => (dimension.isGrade ? variant.gradeSlug === option.key : attributeValuesOnVariant(variant, dimension.key).includes(option.key))),
	);
}
...
export function filterOptionsForGrade(dimension: Dimension, variants: Variant[], gradeSlug: string): DimensionOption[] {
...
export function gradeOptionState(optionKey: string, variants: Variant[], currentSelection: Record<string, string>): OptionStateValue {
...
export function formatDimensionOverview(dimensions: Dimension[], variants: Variant[], gradeSlug: string): string | null {
...
export function formatConfigurationSummary(dimensions: Dimension[], selection: Record<string, string>, gradeLabel?: string): string | null {
	const parts: string[] = [];
	const gradeKey = selection[GRADE_DIMENSION_KEY];
	if (gradeKey) {
		const gradeDimension = dimensions.find((dimension) => dimension.isGrade);
		parts.push(gradeDimension ? optionLabelForKey(gradeDimension, gradeKey) : (gradeLabel ?? gradeKey));
	}
