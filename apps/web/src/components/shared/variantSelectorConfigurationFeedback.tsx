  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <Sparkles
          size={15}
          className="mt-0.5 shrink-0 text-[var(--color-accent-700)]"
          aria-hidden
        />
        <div className="min-w-0">
          <p className="text-[9.5px] font-semibold uppercase tracking-[0.2em] text-[var(--color-accent-800)] md:text-[10px]">
            You have selected
          </p>
          <p className="mt-0.5 text-[13px] font-semibold leading-snug text-[var(--color-ink-900)] md:text-sm">
            {summary}
          </p>
        </div>
      </div>
...
export function ConfigurationRealignmentNotice({
  message,
}: ConfigurationRealignmentNoticeProps) {
  return (
    <div
      role="status"
      aria-live="assertive"
