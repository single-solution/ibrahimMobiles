...
/**
 * Filter sidebar — grade, brand, and attribute facets. Sort UI removed; server defaults to newest.
 */
...
export type ShopFilterDataProps = Pick<FilterSidebarProps, "categorySlug" | "brands" | "gradeCounts" | "initialFacets">;

export function FilterSidebar({ categorySlug, brands = [], gradeCounts = {}, initialFacets = [] }: FilterSidebarProps) {
	...
	return (
		<>
			...
			<aside className="hidden md:sticky md:top-[calc(var(--desktop-header-h)+24px)] md:block md:h-[calc(100dvh-var(--desktop-header-h)-48px)]">
				<FilterPanel categorySlug={categorySlug} brands={brands} gradeCounts={gradeCounts} initialFacets={initialFacets} />
			</aside>
			...
			>
				<FilterPanel categorySlug={categorySlug} brands={brands} gradeCounts={gradeCounts} initialFacets={initialFacets} isMobile />
			</BottomSheet>
		</>
	);
}
...
export function FilterPanel({
	...
	const visibleGrades = useMemo(() => {
		...
		const scoped = categorySlug ? allGrades.filter((descriptor) => descriptor.categorySlug === categorySlug) : allGrades;
		return [...scoped]
			.filter((descriptor) => (gradeCounts[descriptor.slug] ?? 0) > 0 || selected.has(descriptor.slug))
				...
				<>
					<FilterGroup title="Grade" reveal={!isMobile && layout === "sidebar"}>
						{visibleGrades.length === 0 ? (
							<p className="px-2 text-[12px] text-[var(--color-ink-500)]">No grades configured yet.</p>
						) : (
							<FilterCheckboxList
