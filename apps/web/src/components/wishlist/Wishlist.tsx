      ...
      <ul className="mt-5 grid gap-3 md:mt-6 md:grid-cols-2 md:gap-4">
        {wishlist.items.map((saved) => (
          <WishlistRow
            key={saved.productId}
            saved={saved}
            onRemove={() => wishlist.remove(saved.productId)}
          />
        ))}
      </ul>
    </div>
...
interface WishlistRowProps {
  saved: ReturnType<typeof useWishlist>["items"][number];
  onRemove: () => void;
}
        ...
        <div className="flex flex-1 gap-3 p-3 md:gap-4 md:p-4">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="line-clamp-1 text-[10px] md:text-[11px]">
                  ...
                  <span className="text-[var(--color-ink-500)]">{categoryLabel}</span>
                </p>
                <Link
                  href={href}
              ...
              <button
                type="button"
                onClick={onRemove}
                aria-label={`Remove ${saved.name} from wishlist`}
            ...
            <p className="mt-auto pt-2 text-[16px] font-semibold leading-none tracking-tight text-[var(--color-ink-900)] md:text-[17px]">
              {saved.fromPriceRupees > 0
                ? formatPrice(saved.fromPriceRupees)
                : "Price on request"}
            </p>
          </div>
        </div>
