...
export const OtpInput = forwardRef<HTMLInputElement, OtpInputProps>(function OtpInput(
  { value, onChange, length, label, error, disabled, autoFocus, onComplete },
  ref,
) {
  ...
  return (
    <div>
      {label && (
        <span className="mb-2 block text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-ink-500)]">
          {label}
        </span>
      )}
      ...
      {error && (
        <span
          role="alert"
          className="mt-2 block text-center text-[12.5px] font-medium text-[var(--color-danger-700)]"
        >
          {error}
        </span>
      )}
