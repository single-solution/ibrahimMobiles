...
export function PaymentInstructionsCard({ payment, orderNumber, totalRupees }: PaymentInstructionsCardProps) {
	...
	return (
		<Card className="overflow-hidden">
			<div className="border-b border-[var(--color-ink-100)] bg-[var(--color-warn-50)] px-4 py-3 md:px-5">
				<p className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[var(--color-warn-800)]">Next step</p>
			...
			{copy.accountDetails.length > 0 ? (
				<div className="border-t border-[var(--color-ink-100)] bg-[var(--color-canvas-deep)] p-4 md:p-5">
					<p className="mb-2 text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[var(--color-ink-500)]">Send to</p>
	...
	return (
		<li className="flex items-start justify-between gap-3 rounded-[var(--radius-sm)] bg-[var(--color-canvas)] px-3 py-2 text-[12.5px]">
			<span className="min-w-0 flex-1">
				<span className="block text-[10.5px] font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-500)]">{detail.label}</span>
				<span className="mt-0.5 block break-words font-mono text-[12.5px] text-[var(--color-ink-900)]">{detail.value}</span>
			</span>
			<button
				type="button"
				onClick={handleCopy}
