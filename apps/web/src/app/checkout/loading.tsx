      ...
      <div className="mt-5 grid gap-6 md:mt-8 md:grid-cols-[1fr_360px] lg:grid-cols-[1fr_400px] lg:gap-8">
        <div className="space-y-3 md:space-y-4">
          {Array.from({ length: 3 }).map((_, index) => (
            <section
              key={index}
              className="space-y-3 rounded-[var(--radius-2xl)] border border-[var(--color-ink-100)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-sm)] md:p-5"
            >
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {Array.from({ length: index === 1 ? 4 : 2 }).map((_, fieldIndex) => (
