...
export function GradesByCategoryTabs({ groups, variant }: GradesByCategoryTabsProps) {
	const [activeCategorySlug, setActiveCategorySlug] = useState(() => groups[0]?.categorySlug ?? "");

	const activeGroup = useMemo(() => groups.find((group) => group.categorySlug === activeCategorySlug) ?? groups[0], [groups, activeCategorySlug]);
	...
	return (
		<div className={variant === "desktop" ? "space-y-4" : "mt-8 space-y-4"}>
          ...
          class on the grid runs its cross-fade-up keyframe each time. */}
			<GradeCardGrid key={activeGroup.categorySlug} grades={activeGroup.grades} variant={variant} panelId={activeGroup.categorySlug} />
		</div>
	);
}

function GradeCardGrid({ grades, variant, panelId }: { grades: GradeCategoryGroup["grades"]; variant: "mobile" | "desktop"; panelId: string }) {
	if (variant === "mobile") {
		return (
			<ul role="tabpanel" id={`grades-panel-${panelId}`} className="animate-tab-panel-in reveal-scroll-list grid grid-cols-2 gap-2.5">
               ...
               outer 20 = --radius-xl. */
					...
					>
						<GradeBadge categorySlug={descriptor.categorySlug} gradeSlug={descriptor.slug} size="sm" surface="dark" />
	...
	return (
		<div role="tabpanel" id={`grades-panel-${panelId}`} className="animate-tab-panel-in reveal-scroll-list grid grid-cols-3 gap-3">
			{grades.map((descriptor) => (
				<div
					key={`${descriptor.categorySlug}:${descriptor.slug}`}
...
function GradeCardCopy({ descriptor, variant }: { descriptor: GradeCategoryGroup["grades"][number]; variant: "mobile" | "desktop" }) {
	const textClass =
		variant === "mobile"
