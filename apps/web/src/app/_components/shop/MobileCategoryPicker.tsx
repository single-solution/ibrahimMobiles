...
/** Compact mobile category switcher — one trigger opens a bottom sheet list. */
		...
		<>
			<div className="flex-1">
				<button
					...
					className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full border border-[var(--color-ink-200)] bg-[var(--color-surface)] px-3 text-[13px] font-medium text-[var(--color-ink-800)] transition-colors active:bg-[var(--color-canvas-deep)]"
				...
				>
					...
					<span className="truncate">{activeCategory?.label ?? "Category"}</span>
					<ChevronDown
						size={12}
						aria-hidden
...
export function MobileCategoryPickerSkeleton() {
	return <Skeleton shape="pill" className="h-9 flex-1" />;
}
