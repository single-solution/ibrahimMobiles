		...
		() =>
			...
			gradeSlugs.map((slug) => {
				...
				const descriptor = allGrades.find(
					...
					(entry) => entry.categorySlug === categorySlug && entry.slug === slug,
	...
	return (
		...
		<div className="flex min-w-0 max-w-full flex-wrap items-center justify-end gap-2 md:gap-2.5">
			...
			<FilterDropdown
				...
				label={buildDropdownLabel("Grade", gradeLabels)}

				activeCount={countMultiParam(params, FILTER_PARAM_KEYS.grades)}
			...
			<FilterDropdown
				...
				label={buildDropdownLabel("Brand", brandLabels)}

				activeCount={countMultiParam(params, FILTER_PARAM_KEYS.brands)}
				...
				<FilterDropdown
					...
					label={
						...
						attributeFilterCount > 0
							...
							? `More filters (${attributeFilterCount})`
