...
type ProviderApiKeySettings = Pick<
  ChatAssistantRuntimeSettings,
  "providerApiKeyOpenai" | "providerApiKeyGoogle" | "providerApiKeyAnthropic"
>;
...
export function isAssistantConfigured(
  ...
  return (
    ...
    isAssistantProviderConfigured("anthropic")
  );
}
...
export async function generateAssistantReply(
  ...
  const messages: AssistantChatMessage[] = [
    ...
    ...input.history.map((turn): AssistantChatMessage =>
      turn.role === "assistant"
  ...
  while (result && result.toolCalls.length > 0 && round < MAX_TOOL_ROUNDS) {
    ...
    const outputs = await Promise.all(
      result.toolCalls.map((call) => executeAssistantTool(call, toolContext)),
    );
