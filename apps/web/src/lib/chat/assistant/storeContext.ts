...
export function formatCatalogLine(product: Product): string {
	...
	const priceText = cheapestByGrade.size > 0 ? [...cheapestByGrade.entries()].map(([grade, price]) => `${grade} from ${formatPrice(price)}`).join(", ") : "price on request";
...
export async function buildAssistantStoreContext(input: {
	...
	const policies = [
		...
		paymentMethods.length > 0 ? `Payment methods (complete at checkout — never share account numbers): ${paymentMethods.join(", ")}.` : "",
