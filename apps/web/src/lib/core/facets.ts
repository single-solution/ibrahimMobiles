...
function resolveFacetLabel(
  ...
  const display = customLabel?.trim()
    ? { [attribute.slug]: customLabel.trim() }
    : undefined;
  return resolveVariantAttributeLabel(
...
export async function getFacets(
  ...
  const visibleNodes = sortAttributesByVisibility(
    descriptors
      .filter((attribute) =>
        isVisibilitySatisfied(
          attribute.visibility as AttributeVisibility | undefined,
          visibilityContext,
        ),
      )
      .map((attribute) => ({
  ...
  const visible = visibleNodes
    .map((node) => descriptors.find((row) => row.slug === node.slug))
    .filter((row): row is AttributeDescriptor => Boolean(row));
      ...
      ? [
          {
            $match: Object.fromEntries(
              Object.entries(variantMatch).map(([key, value]) => [
                `variants.${key}`,
                value,
              ]),
            ),
  ...
  return visible.map((attribute) => {
    ...
    for (const [compound, count] of counts) {
      ...
      options.push({
        value,
        label: resolveFacetLabel(
          attribute,
          value,
          customLabels.get(compound),
        ),
        count,
        backgroundColor: attribute.options.find((option) => option.value === value)
          ?.backgroundColor,
      });
    }
