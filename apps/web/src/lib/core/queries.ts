 ...
 *
 * Categories and grades with `isActive: true` are included in storefront visibility.
 */
 ...
 */
export async function getGradeCounts(categorySlug: string): Promise<Record<string, number>> {
	await connectDB();
 ...
 * selector's grade hint. Admin grade edits appear on the storefront within
 * the cache TTL.
 */
export async function getGrades(): Promise<GradeDescriptor[]> {
	await connectDB();
	const grades = await GradeModel.find({ isActive: true }).sort({ categorySlug: 1, label: 1 }).lean<GradeLean[]>();
	if (grades.length === 0) {
		logger.warn("getGrades: no grades in DB; storefront grade UI will render empty");
	}
