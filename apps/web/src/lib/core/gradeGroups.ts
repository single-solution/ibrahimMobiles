...
export function buildGradeCategoryGroups(grades: GradeDescriptor[], categories: CategorySortInput[]): GradeCategoryGroup[] {
	const labelBySlug = new Map(categories.map((category) => [category.slug, category.label]));
	const orderBySlug = new Map(categories.map((category) => [category.slug, category.sortOrder ?? 0]));
	...
	return Array.from(gradesByCategory.entries())
		...
		.sort((left, right) => (orderBySlug.get(left.categorySlug) ?? 0) - (orderBySlug.get(right.categorySlug) ?? 0));
}
