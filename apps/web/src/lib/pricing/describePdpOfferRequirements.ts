...
const STOREFRONT_SCOPE_CONDITION_TYPES = new Set<OfferCondition["type"]>(["categories", "brands", "grades", "products", "attributes", "price_range"]);
...
function resolveGradeLabels(gradeSlugs: string[], grades: GradeDescriptor[]): string {
	return gradeSlugs.map((slug) => grades.find((grade) => grade.slug === slug)?.label ?? humanizeSlug(slug)).join(" or ");
}
...
function formatRequirementRow(condition: OfferCondition, context: PdpOfferRequirementsContext): PdpOfferRequirementRow | null {
	...
	if (condition.type === "grades") {
		const gradeSlugs = asStringArray(condition.value);
		if (gradeSlugs.length === 0) {
			return null;
		}
		return {
			label: "Grade",
			value: resolveGradeLabels(gradeSlugs, context.grades),
		};
	}
...
const CONDITION_DISPLAY_ORDER: OfferCondition["type"][] = ["categories", "brands", "products", "grades", "attributes", "price_range"];

const REQUIREMENT_ROW_ORDER = ["Category", "Brand", "Product", "Grade", "Price", "Quantity"] as const;
