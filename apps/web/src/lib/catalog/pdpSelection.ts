...
const RESERVED_PDP_PARAMS = new Set([PDP_GRADE_PARAM, "compare"]);
...
export function variantsForGrade(variants: Variant[], gradeSlug: string): Variant[] {
	if (!gradeSlug) {
		return variants;
	}
...
export function mergeSelectionForGradeChange(previousSelection: Record<string, string>, nextGradeSlug: string, variants: Variant[]): Record<string, string> {
	...
	for (const [key, value] of Object.entries(previousSelection)) {
		...
		const stillValid = gradeScoped.some((variant) => attributeValuesOnVariant(variant, key).includes(value));
		if (stillValid) {
			merged[key] = value;
		}
	}
...
export function resolvePickerSelection(variants: Variant[], selection: Record<string, string>, priorityKey?: string): { variant: Variant; selection: Record<string, string> } {
	...
	const tryOrder = priorityKey ? [priorityKey, ...keys.filter((key) => key !== priorityKey)] : [...keys.filter((key) => key !== GRADE_DIMENSION_KEY), GRADE_DIMENSION_KEY];
...
export function parsePdpSelectionFromSearch(search: { [key: string]: string | string[] | undefined }, categoryAttributeSlugs: string[]): Record<string, string> {
	...
	const grade = typeof search[PDP_GRADE_PARAM] === "string" ? search[PDP_GRADE_PARAM] : "";
	if (grade) {
