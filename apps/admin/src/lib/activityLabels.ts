...
export function resolveResourceUrl(type: AdminActivityResourceType, id?: string): string | null {
  if (!id) return null;
  switch (type) {
    case "order": return `/orders?order=${id}`;
    case "customer": return `/customers?customer=${id}`;
    case "product": return `/products?product=${id}`;
    case "team": return `/team?member=${id}`;
    case "inquiry": return `/inquiries?inquiry=${id}`;
    case "loyalty": return `/settings?tab=loyalty`;
    case "settings": return `/settings?tab=${id}`;
