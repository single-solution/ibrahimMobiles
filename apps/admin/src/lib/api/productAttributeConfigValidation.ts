...
type ValidationResult =
	...
	| { ok: true; value: ProductAttributeConfig }
...
export async function validateProductAttributeConfig(
	...
	if (
		...
		input.attributeSlugs === undefined &&

		input.attributeOptionPool === undefined &&

		input.attributeCustomOptions === undefined &&

		input.attributeDefaults === undefined
	...
	if (input.attributeOptionPool !== undefined) {
		...
		if (
			...
			input.attributeOptionPool === null ||

			typeof input.attributeOptionPool !== "object" ||

			Array.isArray(input.attributeOptionPool)
		...
		for (const [rawSlug, rawValues] of Object.entries(

			input.attributeOptionPool as Record<string, unknown>,
			...
			const customValues = new Set(
				...
				(attributeCustomOptions[slug] ?? []).map((option) => option.value.toLowerCase()),
	...
	if (input.attributeDefaults !== undefined) {
		...
		if (
			...
			input.attributeDefaults === null ||

			typeof input.attributeDefaults !== "object" ||

			Array.isArray(input.attributeDefaults)
		...
		for (const [slug, rawValue] of Object.entries(

			input.attributeDefaults as Record<string, unknown>,
	...
	const resolvedSlugs =
		...
		[
			...
			...new Set([
				...
				...Object.keys(attributeOptionPool),

				...Object.keys(attributeCustomOptions),

				...Object.keys(attributeDefaults),
	...
	const useStrictPools =
		options.strictPools === true &&
		input.attributeSlugs !== undefined &&
		input.attributeOptionPool !== undefined;
	...
	for (const slug of resolvedSlugs) {
		...
		config.attributeOptionPool[slug] = Object.prototype.hasOwnProperty.call(
			attributeOptionPool,
			slug,
		)
