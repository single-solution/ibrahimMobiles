...
export function isPerformanceRange(value: string | undefined): value is PerformanceRange {
  return (
    value !== undefined && (PERFORMANCE_RANGES as readonly string[]).includes(value)
  );
}

export function isPerformanceCompare(
  value: string | undefined,
): value is PerformanceCompare {
  return (
    value !== undefined && (PERFORMANCE_COMPARES as readonly string[]).includes(value)
  );
}
