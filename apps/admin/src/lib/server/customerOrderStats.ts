...
export async function loadCustomerOrderStats(
  ...
  const [stats, loyalty] = await Promise.all([
    ...
    LoyaltyAccount.findOne({ customerId })
      .select({ balance: 1, lifetimeEarned: 1 })
      .lean<{ balance?: number; lifetimeEarned?: number } | null>(),
  ]);
