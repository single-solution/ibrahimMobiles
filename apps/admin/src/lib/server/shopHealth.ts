  ...
  const rows = await Product.aggregate<ProductHealthAgg>([
    {
      $facet: {
        ...
        productsArchivedButFeatured: [
          { $match: { isFeatured: true, isArchived: true } },
          { $count: "n" },
        ],
          ...
          { $unwind: "$variants" },
          {
            $group: {
              ...
              variantsOutOfStock: {
                $sum: {
                  $cond: [
                    { $lte: [{ $ifNull: ["$variants.quantity", 0] }, 0] },
                    1,
                    0,
                  ],
                },
...
function evaluateSettings(settings: StoreSettings): ShopHealthCheck[] {
  ...
  const enabledMethods = [
    settings.paymentBankEnabled,
    settings.paymentEasypaisaEnabled,
    settings.paymentJazzcashEnabled,
  ...
  if (
    settings.paymentBankEnabled &&
    !(
      settings.paymentBankName.trim() &&
      (settings.paymentBankAccountNumber.trim() || settings.paymentBankIban.trim())
    )
  ) {
    checks.push({
...
function evaluateProducts(
  ...
  if (agg.productsWithoutImages > 0) {
    checks.push({
      ...
      title: `${agg.productsWithoutImages} ${pluralise(
        agg.productsWithoutImages,
        "product",
      )} without images`,
  ...
  if (agg.productsWithoutVariants > 0) {
    checks.push({
      ...
      title: `${agg.productsWithoutVariants} ${pluralise(
        agg.productsWithoutVariants,
        "product",
      )} with no buyable variants`,
  ...
  if (agg.productsArchivedButFeatured > 0) {
    checks.push({
      ...
      title: `${agg.productsArchivedButFeatured} archived ${pluralise(
        agg.productsArchivedButFeatured,
        "product",
      )} still marked as featured`,
  ...
  if (agg.variantsOutOfStock > 0) {
    checks.push({
      ...
      title: `${agg.variantsOutOfStock} ${pluralise(
        agg.variantsOutOfStock,
        "variant",
      )} out of stock`,
  ...
  if (agg.variantsLowStock > 0) {
    checks.push({
      id: "stock-low-stock",
      title: `${agg.variantsLowStock} ${pluralise(
        agg.variantsLowStock,
        "variant",
      )} low on stock`,
      description: `Stock at or below ${lowStockThreshold} ${pluralise(
        lowStockThreshold,
        "unit",
      )}.`,
      severity: "info",
...
export async function loadShopHealth(): Promise<ShopHealthSummary> {
  ...
  const checks = [...settingsChecks, ...productChecks].sort(
