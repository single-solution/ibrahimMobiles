...
export type { PerformanceCompare, PerformanceRange };
export {
 ...
 * on every mutation.
 */
export async function loadPerformanceSummary(args: {
  range: PerformanceRange;
  compare: PerformanceCompare;
  ...
  const averageOrderRupees =
    current.count > 0 ? Math.round(current.totalRupees / current.count) : 0;
  const previousAverage =
    previous.count > 0 ? Math.round(previous.totalRupees / previous.count) : 0;
