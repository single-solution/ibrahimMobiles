...
function toVariantResponse(variant: VariantAttributes & { isActive?: boolean }): AdminVariant {
  return {
    ...
    forceOutOfStock:
      variant.forceOutOfStock === true || variant.isActive === false,
...
function computeVariantRollup(product: ProductLean, images: StoredImage[]) {
  ...
  const inStockCount = variants.filter((variant) => {
    const legacy = variant as VariantAttributes & { isActive?: boolean };
    return (
      legacy.forceOutOfStock !== true &&
...
function asCustomOptionsRecord(
  raw: unknown,
...
function toProductAttributeConfigFields(product: ProductLean) {
  const attributeSlugs = Array.isArray(product?.attributeSlugs)
    ? product.attributeSlugs.filter(
  ...
  return {
    ...
    ...(attributeSlugs !== undefined
      ? { attributeOptionPool: attributeOptionPool ?? {} }
      : attributeOptionPool !== undefined
        ? { attributeOptionPool }
        : {}),
    ...(attributeSlugs !== undefined
      ? { attributeCustomOptions: attributeCustomOptions ?? {} }
