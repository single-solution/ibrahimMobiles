  ...
  const visibleOptions = useMemo(() => {
    ...
    return LUCIDE_ICON_OPTIONS.filter(
      (option) =>
        ...
        option.keywords.includes(trimmedQuery),
    );
  }, [trimmedQuery]);

  return (
    <div className="rounded-md border border-[var(--color-ink-100)] bg-[var(--color-surface)] p-3">
      <p className="px-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-500)]">
        {label}
      </p>
      <div className="mt-2 flex items-center gap-3">
        <span
          className="grid size-12 place-items-center rounded-md border border-[var(--color-ink-200)] bg-[var(--color-canvas-deep)] text-[var(--color-ink-700)]"
          aria-hidden
        >
        ...
        <p className="min-w-0 flex-1 truncate text-[12px] text-[var(--color-ink-500)]">
          {value}
        </p>
      </div>
      {isOpen && (
