...
export function AdminAlertPill({
  href,
  label,
  tone = "neutral",
  onClick,
...
export function AdminAlertsRow({
  alerts,
  onNavigate,
  emptyLabel = "All clear — no alerts right now",
  className,
}: AdminAlertsRowProps) {
  ...
  if (!hasAlerts) {
    return (
      <span
        className={classNames(
          "inline-flex shrink-0 items-center gap-1 text-[var(--color-ink-500)]",
          className,
        )}
      >
        <span
          className="grid size-1.5 place-items-center rounded-full bg-emerald-500"
          aria-hidden
        />
        {emptyLabel}
      </span>
    ...
    >
      {alerts.unreadInquiries > 0 && can("inquiry_view") ? (
        <AdminAlertPill
          ...
          tone="danger"
          onClick={onNavigate}
          label={`${alerts.unreadInquiries} unread inquir${alerts.unreadInquiries === 1 ? "y" : "ies"}`}
        />
      ) : null}
      ...
      can("inquiry_view") ? (
        <AdminAlertPill
          ...
          tone="neutral"
          onClick={onNavigate}
          label={`${alerts.openInquiries} open inquir${alerts.openInquiries === 1 ? "y" : "ies"}`}
        />
      ) : null}
    </div>
  );
