...
export function StructuredContentCompactPreview({
  content,
  fallback,
  className,
  clampLines,
  ...
  const classes = [className, clampLines ? CLAMP_CLASS[clampLines] : null]
    .filter(Boolean)
    .join(" ");
...
export function StructuredContentFullPreview({
  ...
  const visibleBullets =
    typeof maxBullets === "number" && maxBullets >= 0
      ? bullets.slice(0, maxBullets)
      : bullets;
...
export function previewHasRenderableContent(
  content?: StructuredContent,
  fallback?: string,
): boolean {
