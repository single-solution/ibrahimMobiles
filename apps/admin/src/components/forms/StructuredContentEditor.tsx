...
export function StructuredContentEditor({
  ...
  function updateBullet(index: number, patch: Partial<StructuredContentBullet>) {
    const nextBullets = bullets.map((bullet, i) =>
    ...
    <div className="space-y-4 rounded-md border border-[var(--color-ink-100)] bg-[var(--color-canvas-deep)]/40 p-3">
      <div>
        <div className="mb-1 flex items-baseline justify-between gap-2">
          <label
            htmlFor="structured-content-summary"
      ...
      <div>
        <div className="mb-1 flex items-baseline justify-between gap-2">
          <p className="text-[11.5px] font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-700)]">
            {bulletsLabel}
          </p>
          <span className="text-[10.5px] text-[var(--color-ink-400)]">
        ...
        <ul className="space-y-2">
          {bullets.map((bullet, index) => (
            <li
              key={index}
              className="flex items-center gap-2 rounded-md border border-[var(--color-ink-100)] bg-[var(--color-surface)] p-2"
            >
              <InlineIconPicker
                value={bullet.icon}
                onChange={(icon) => updateBullet(index, { icon })}
              />
              <input
                type="text"
                value={bullet.text}
    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          className="fixed inset-0 z-[95] flex items-center justify-center bg-[var(--color-ink-900)]/35 p-4"
          onClick={(event) => {
            if (event.target === event.currentTarget) setIsOpen(false);
          }}
        >
          <div className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-[var(--radius-lg)] border border-[var(--color-ink-100)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-lg)]">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-[var(--color-ink-500)]">
                  Any lucide icon works — the storefront falls back to the
                  surface default if data is ever missing.
                </p>
              </div>
              <button
                type="button"
                ...
                <button
                  ...
                  className={
                    ...
                    (name === value
                      ...
                      : "border-[var(--color-ink-100)] bg-[var(--color-canvas-deep)]")
                  }
                >
