...
export function TextField({ label, hint, errorText, leadingIcon, trailingAddon, containerClassName, id, className, ...rest }: TextFieldProps) {
	...
	return (
		<div className={classNames("reveal animate-in flex flex-col gap-1.5", containerClassName)}>
			...
			>
				{Boolean(leadingIcon) && <span className="text-[var(--color-ink-400)]">{leadingIcon}</span>}
				<input
					id={inputId}
					{...rest}
					className={classNames(
				...
				/>
				{Boolean(trailingAddon) && <span className="text-xs font-medium text-[var(--color-ink-500)]">{trailingAddon}</span>}
			</div>
