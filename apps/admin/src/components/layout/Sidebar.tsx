      ...
      <nav className="flex-1 overflow-y-auto py-2.5">
        {SIDEBAR_SECTIONS.map((section) => (
          <div key={section.title} className="mb-3">
            {!isCollapsed && (
              <p className="px-3 pb-1 text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-400)]">
                {section.title}
              </p>
            )}
      ...
      <footer className="shrink-0 border-t border-[var(--color-ink-100)] p-2">
        <Link
          ...
          className={classNames(
            ...
            isCollapsed
              ? "mx-auto size-9 shrink-0 justify-center rounded-[var(--radius-md)]"
              : "h-9 w-full rounded-[var(--radius-md)] px-3",
          )}
        >
