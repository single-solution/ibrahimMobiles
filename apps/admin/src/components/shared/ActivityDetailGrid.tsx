  ...
  return (
    <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
      {rows.map((row, i) => (
        ...
        >
          <span className="shrink-0 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-ink-500)] sm:w-28">
            {row.key}
          </span>
          <span className="min-w-0 break-words font-medium text-[var(--color-ink-900)]">
            {row.value}
          </span>
        </div>
      ))}
