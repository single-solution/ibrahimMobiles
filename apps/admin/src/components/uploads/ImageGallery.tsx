...
export function ImageGallery({
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < accepted.length; i++) {
        ...
        const imageAltText = altTextBase
          ? `${altTextBase} · image ${altIndex}`
          : undefined;
        try {
          const pending = await createPendingImageDraft(
            file,
            imageAltText ?? file.name.replace(/\.[^.]+$/, ""),
          );
          additions.push(pending);
        } catch (uploadError) {
          setError(
            uploadError instanceof Error
              ? `Failed to preview "${file.name}": ${uploadError.message}`
              : `Failed to preview "${file.name}"`,
          );
        }
      }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {lightboxIndex !== null && (
        <Lightbox
          urls={value.map((image) => getImageDraftUrl(image, "full"))}
