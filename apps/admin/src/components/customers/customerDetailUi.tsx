...
export function CustomerErrorBanner({
  message,
  onDismiss,
}: {
  message: string;
  ...
  return (
    <div
      role="alert"
      className="flex items-start justify-between gap-2 rounded-[var(--radius-md)] border border-rose-100 bg-rose-50 px-3 py-2 text-xs text-rose-800"
    >
      ...
      {onDismiss ? (
        <button
          ...
          onClick={onDismiss}
          className="shrink-0 rounded p-0.5 text-rose-600 hover:bg-rose-100"
        >
          <X size={12} />
        </button>
      ) : null}
