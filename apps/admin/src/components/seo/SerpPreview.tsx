...
export function SerpPreview({ resolved, siteUrl }: SerpPreviewProps) {
  ...
  return (
    <div
      className="rounded-[var(--radius-md)] border border-[var(--color-ink-100)] bg-white p-4"
    ...
    >
      ...
      <div className="mt-3 font-[Arial,Helvetica,sans-serif]">
        <div className="flex items-center gap-2 text-sm leading-none text-[#202124]">
          <span className="grid size-7 shrink-0 place-items-center rounded-full bg-[var(--color-canvas-deep)] text-[10px] font-semibold text-[var(--color-ink-600)]">
            G
          </span>
          <div className="min-w-0">
            <p className="truncate text-sm text-[#202124]">{breadcrumb.split(" › ")[0]}</p>
            <p className="truncate text-xs text-[#4d5156]">{breadcrumb}</p>
          </div>
        </div>
        <p className="mt-1 text-xl leading-snug text-[#1a0dab] hover:underline">
          {title}
        </p>
        <p className="mt-0.5 text-sm leading-snug text-[#4d5156]">{description}</p>
      </div>
    </div>
  );
}
