        ...
        <aside className="hidden w-full shrink-0 flex-col border-r border-[var(--color-ink-100)] bg-[var(--color-canvas)] p-3 lg:flex lg:w-[min(220px,28%)] lg:max-w-xs">
          ...
          <div className="mt-4 space-y-2">
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                key={index}
                className="h-8 animate-pulse rounded-[var(--radius-md)] bg-[var(--color-ink-100)]/70"
              />
            ))}
          </div>
        </aside>
        <section className="flex min-h-0 flex-1 flex-col border-r border-[var(--color-ink-100)] lg:max-w-sm">
          ...
          <div className="flex-1 space-y-2 p-3">
            {Array.from({ length: 8 }).map((_, index) => (
              <div
                key={index}
                className="h-[72px] animate-pulse rounded-[var(--radius-lg)] bg-[var(--color-ink-100)]/70"
              />
            ))}
          </div>
        </section>
        <section className="hidden min-h-0 flex-1 flex-col lg:flex">
          ...
          <div className="flex-1 space-y-3 p-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="h-16 animate-pulse rounded-[var(--radius-lg)] bg-[var(--color-ink-100)]/70"
              />
            ))}
          </div>
        </section>
