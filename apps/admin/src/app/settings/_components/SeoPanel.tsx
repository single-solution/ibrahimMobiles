    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        >
          <input
            ...
            onChange={(event) =>
              set(
                "focusKeyword",
                clampLength(event.target.value, SEO_META_FIELD_LIMITS.focusKeyword),
              )
            }
        ...
        >
          <input
            ...
            value={value.title ?? ""}
            onChange={(event) =>
              set("title", clampLength(event.target.value, SEO_META_FIELD_LIMITS.title))
            }
      ...
      >
        <textarea
          ...
          onChange={(event) =>
            set(
              "description",
              clampLength(event.target.value, SEO_META_FIELD_LIMITS.description),
            )
          }
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <FieldRow
          ...
          hint="Points to the preferred version of this page to avoid duplicate content."
        >
          <input
            id={`${id}-canonical`}
            type="url"
            ...
            onChange={(event) =>
              set(
                "canonicalUrl",
                clampLength(event.target.value, SEO_META_FIELD_LIMITS.canonicalUrl),
              )
            }
        ...
        <FieldRow
          label="OG image URL"
          htmlFor={`${id}-og`}
          hint="Custom image URL for social media sharing."
        >
          <input
            id={`${id}-og`}
            type="url"
            ...
            onChange={(event) =>
              set(
                "ogImageUrl",
                clampLength(event.target.value, SEO_META_FIELD_LIMITS.ogImageUrl),
              )
            }
      ...
      <div className="flex flex-wrap items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          <Toggle
            ...
            onCheckedChange={(checked) => set("nofollow", checked)}
            aria-label="Ignore links (nofollow)"
          />
          <div>
...
function FieldRow({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {children}
      {hint ? (
        <p className="text-[11px] text-[color:var(--color-ink-400)]">{hint}</p>
      ) : null}
