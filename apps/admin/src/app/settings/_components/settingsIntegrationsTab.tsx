...
function pixelStatus(
  value: string,
  pattern: RegExp,
): { tone: SettingsHeroMetricTone; label: string; validation?: string } {
...
export function IntegrationsSettings({ draft, saved, setField, onSaved, canUpdate }: SectionProps) {
  const fields = useMemo(
    () => [...STORE_SETTING_GROUPS.social, ...STORE_SETTING_GROUPS.marketing],
    [],
  );

  const socialLinks = [
    draft.socialFacebook,
    draft.socialInstagram,
    draft.socialTiktok,
    draft.socialYoutube,
    draft.socialGoogleMaps,
  ];
  ...
  return (
    ...
    >
      <FormSection
        ...
        description="Linked from the footer and About page. Leave a row blank to hide that platform."
      >
        <FormGrid cols={3}>
          <TextField
            label="Facebook URL"
