	...
	try {
		...
		if (search) {
			searchedCustomers = await Customer.find({
				$or: [{ name: { $regex: searchPattern, $options: "i" } }, { phoneNumber: { $regex: searchPattern, $options: "i" } }],
			})
				.select("_id name")
				.lean<CustomerLean[]>();
		...
		const [accounts, total] = await Promise.all([
			LoyaltyAccount.find(accountFilter).sort({ balance: -1 }).skip(skip).limit(limit).lean<LoyaltyAccountLean[]>(),
			LoyaltyAccount.countDocuments(accountFilter),
		]);

		const customerNameById = await resolveCustomerNames(searchedCustomers, accounts);

		const items: AdminLoyaltyAccount[] = accounts.map((account) => toLoyaltyAccountResponse(account, customerNameById.get(account.customerId.toString()) ?? "Customer"));
 ...
 */
async function resolveCustomerNames(searchedCustomers: SearchedCustomer[] | null, accounts: LoyaltyAccountLean[]): Promise<Map<string, string>> {
	if (searchedCustomers) {
		return new Map(searchedCustomers.map((customer) => [customer._id.toString(), customer.name]));
	}
