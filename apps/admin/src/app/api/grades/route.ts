...
import { badRequest, conflict, created, isValidationError, normalizeStructuredContent, ok, parseBody, slugify, validateString } from "@store/shared";
...
async function hasGradeCategoryConflict(categorySlug: string, slug: string): Promise<boolean> {
...
export async function GET(request: Request) {
	...
	const [docs, total] = await Promise.all([Grade.find(filter).sort({ categorySlug: 1, label: 1 }).skip(skip).limit(limit).lean<GradeLean[]>(), Grade.countDocuments(filter)]);
...
export async function POST(request: Request) {
	...
	const color = typeof body.color === "string" && HEX_COLOR_REGEX.test(body.color) ? body.color : "#1f2937";
	const video = typeof body.video === "string" ? body.video.trim() : "";

	const slug = typeof body.slug === "string" && body.slug.trim().length > 0 ? slugify(body.slug, 64) : slugify(labelResult, 64);
	if (slug.length === 0) {
