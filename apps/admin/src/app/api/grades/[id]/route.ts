...
import { cascadeGradeSlugChange, slugFromCatalogLabel } from "@/lib/services/catalogSlugSync";

const HEX_COLOR_REGEX = /^#[0-9a-f]{6}$/i;

async function hasGradeSlugConflict(id: string, categorySlug: string, slug: string): Promise<boolean> {
...
export async function PUT(request: Request, { params }: RouteContext) {
	...
	if (body.content !== undefined) {
		const fallbackSummary = typeof update.notes === "string" ? (update.notes as string) : typeof body.notes === "string" ? body.notes : "";
	...
	await connectDB();
	try {
		const current = await Grade.findById(id).select("categorySlug slug label").lean<{ categorySlug: string; slug: string; label: string }>();
		if (!current) {
		...
		const doc = await Grade.findByIdAndUpdate(id, { $set: update }, { new: true, runValidators: true }).lean<GradeLean>();
		if (!doc) {
			return notFound("Grade not found");
		}

		if (typeof update.slug === "string" && update.slug !== current.slug) {
			await cascadeGradeSlugChange(current.categorySlug, current.slug, update.slug as string);
		}
