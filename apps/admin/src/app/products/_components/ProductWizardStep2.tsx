...
function buildGradeSections(product: AdminProduct, grades: AdminGrade[]): GradeSectionState[] {
...
export function ProductWizardStep2({
	...
	const resolveWorkspaceSelection = useCallback((built: GradeSectionState[], categoryGrades: AdminGrade[], urlGrade: string | null, urlUid: string | null) => {
		const gradeFromUrl = urlGrade && built.some((row) => row.gradeSlug === urlGrade) ? urlGrade : null;
		const firstWithVariants = built.find((row) => row.combinations.length > 0);
		const gradeSlug = gradeFromUrl ?? firstWithVariants?.gradeSlug ?? categoryGrades[0]?.slug ?? null;
		const section = built.find((row) => row.gradeSlug === gradeSlug);
		const uidFromUrl = urlUid && section?.combinations.some((row) => row.uid === urlUid) ? urlUid : null;
		const variantUid = uidFromUrl ?? section?.combinations[0]?.uid ?? null;
	...
	const activeSection = sections.find((section) => section.gradeSlug === selectedGradeSlug);
	const activeVariants = activeSection?.combinations ?? [];
	const selectedVariant = activeVariants.find((row) => row.uid === selectedVariantUid);
	const selectedComboIndex = selectedVariant ? activeVariants.findIndex((row) => row.uid === selectedVariant.uid) : -1;
	...
	function updateSection(gradeSlug: string, updater: (section: GradeSectionState) => GradeSectionState) {
		setSections((prev) => prev.map((section) => (section.gradeSlug === gradeSlug ? updater(section) : section)));
	}
	...
	async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		const result = validateVariantDrafts(variants, surface, product.brand.slug, attributeConfig, (index) => {
		...
		if (!result.ok) {
			...
			const firstInvalid = flatCombinations.find((row) => variantHasErrors(nextErrorMap, `grade.${row.gradeSlug}.combinations.${row.comboIndex}`));
			if (firstInvalid) {
			...
			const gradeLabel = grades.find((row) => row.slug === firstInvalid?.gradeSlug)?.label ?? firstInvalid?.gradeSlug;
			toast.danger(
				firstInvalid
		...
		try {
			...
			const categoryGrades = catalog.gradesByCategory[product.categorySlug] ?? [];
			resetWorkspace(latest, categoryGrades, selectedGradeSlug, selectedVariantUid);
			onSaved(latest);
		} catch (error) {
					...
					<>
						<TabList
							...
							tabs={[
								...
								...sections
									.filter((section) => section.combinations.length > 0 && !grades.some((grade) => grade.slug === section.gradeSlug))
									.map((section) => ({
						...
						<div className="flex min-h-0 flex-1 flex-col sm:flex-row">
							<aside className="flex w-[17.5rem] shrink-0 flex-col border-r border-[var(--color-ink-100)] bg-[var(--color-canvas)] p-2.5 xl:w-80">
								...
								<nav aria-label="Variants in grade" className="-mx-0.5 min-h-0 flex-1 overflow-y-auto">
									{activeVariants.length === 0 ? (
										<p className="px-1 py-2 text-[11px] text-[var(--color-ink-500)]">No variants in this grade.</p>
									) : (
											...
											{activeVariants.map((variant, comboIndex) => {
												...
												return (
													<VariantSidebarTile
														...
														onChange={(next) => updateCombination(gradeSlug, variant.uid, next)}
													/>
												);
											})}
										</ul>
							...
							<div className="flex min-h-0 min-w-0 flex-1 flex-col">
								{!selectedGradeSlug ? (
									<p className="p-3 text-sm text-[var(--color-ink-500)]">Select a grade above.</p>
								) : !selectedVariant ? (
									...
									<>
										...
										{selectedComboIndex >= 0 && variantHasErrors(errorMap, `grade.${selectedGradeSlug}.combinations.${selectedComboIndex}`) ? (
										...
										<div className="min-h-0 flex-1 overflow-y-auto p-3">
											<VariantCard
												...
												onRemove={() => removeCombination(selectedGradeSlug, selectedVariant.uid)}
											/>
										</div>
										<VariantDetailFooter
											variant={selectedVariant}
