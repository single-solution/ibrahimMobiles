...
function hasActiveFilters(filters: ProductFilters): boolean {
	return filters.stock !== "all" || filters.status !== "all" || filters.featured !== "all" || filters.photos !== "all" || filters.brands.length > 0 || filters.grades.length > 0;
}
...
function matchesFilters(product: AdminProductSummary, filters: ProductFilters): boolean {
	...
	if (filters.grades.length > 0) {
		const matchesGrade = filters.grades.some((slug) => product.gradeSlugs.includes(slug));
...
function ProductsCatalogInner({ products, catalog }: ProductsCatalogProps) {
	...
	const gradeOptions = useMemo<FilterOption[]>(() => {
		...
		return grades.map((grade) => ({
			value: grade.slug,
			label: grade.label,
			count: productsMatchingSearch.filter((product) => product.gradeSlugs.includes(grade.slug)).length,
		}));
						...
						<WorkspaceCatalogPaneHeader
							...
							filters={
								<>
									...
									<FilterDropdown label="Grade" options={gradeOptions} selected={filters.grades} onChange={(grades) => updateFilters({ grades })} />
									<FilterDropdown
										label="Stock"
										single
