				...
				<header className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-2">
						...
						<p className="text-[12.5px] font-semibold uppercase tracking-[0.14em] text-[var(--color-ink-700)]">
							...
							{grade && (
								<ColoredPill backgroundColor={grade.color} className="ml-2 rounded-full px-2 py-0.5 text-[10.5px] font-semibold uppercase tracking-[0.1em]">
									{grade.label}
								</ColoredPill>
							)}
						</p>
			...
			{!lockGradeSlug && (
				<div>
					...
					<div className="flex flex-wrap gap-1.5">
						{grades.length === 0 && <p className="text-[12px] italic text-[var(--color-ink-400)]">Define grades for this category first.</p>}
						{[...grades]
							.sort((left, right) => compareAlphabetically(left.label, right.label))
							.map((gradeItem) => {
