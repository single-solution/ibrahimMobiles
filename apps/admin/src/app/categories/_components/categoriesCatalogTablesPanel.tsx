...
import { buildAttributeColumns, buildBrandColumns, buildGradeColumns } from "./categoriesCatalogColumns";
...
export function CategoriesCatalogTablesPanel({
	...
	const gradeColumns = buildGradeColumns(columnHandlers);
						...
						<div className="flex min-w-0 flex-1 items-center gap-1.5 sm:mr-auto">
							...
							<div className="min-w-0">
								...
								<p className="text-[10px] text-[var(--color-ink-500)]">
									{activeTab === "brands" ? filteredBrands.length : activeTab === "grades" ? filteredGrades.length : filteredAttributes.length} shown · slug{" "}
					...
					<>
						...
						{activeTab === "grades" ? (
							<Table
								...
								rowKey={(row) => row.id}
								emptyState={!selectedCategory ? "Select a category from the sidebar." : rowQuery.trim() ? "No items match your search." : "No grades in this category yet."}
							/>
						) : null}
