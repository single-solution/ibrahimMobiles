...
import type { AdminAttribute, AdminAttributeCardPosition, AdminAttributeOption, AdminBrand, AdminCategory, AdminGrade } from "@/types/models";
...
function visibilityFromAttribute(visibility: AttributeVisibility | undefined): Pick<FormState, "visibilityType" | "brandSlugs" | "gradeSlugs"> {
...
export function AttributeEditor({ isOpen, onClose, attribute, category, brands, grades, onSaved }: AttributeEditorProps) {
									...
									return (
										<button
											...
											onClick={() =>
												setForm((prev) => ({
													...prev,
													gradeSlugs: selected ? prev.gradeSlugs.filter((slug) => slug !== grade.slug) : [...prev.gradeSlugs, grade.slug],
												}))
											}
											className={
												"rounded-full border px-2.5 py-1 text-[12px] font-medium " +
