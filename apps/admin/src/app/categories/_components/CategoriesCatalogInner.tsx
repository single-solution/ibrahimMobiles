...
const GradeEditor = dynamic(() => import("./GradeEditor").then((mod) => ({ default: mod.GradeEditor })), { ssr: false });
...
export function CategoriesCatalogInner({ initialCategories, initialBrands, initialGrades, initialAttributes }: CategoriesCatalogInnerProps) {
	...
	}, [searchParams, selectedCategory, categories, brands, grades, attributes]);
	...
	}, [searchParams, categories, brands, grades, attributes, selectedCategory]);
	...
	const gradesForCategory = useMemo(() => {
		if (!selectedCategorySlug) return [];
		return grades.filter((row) => row.categorySlug === selectedCategorySlug).sort((left, right) => compareAlphabetically(left.label, right.label));
	...
	const filteredGrades = useMemo(() => {
		...
		return gradesForCategory.filter((row) => matchesQuery([row.label, row.slug, row.notes].join(" "), rowQuery));
	...
	const newButtonLabel = activeTab === "brands" ? "New brand" : activeTab === "grades" ? "New grade" : "New attribute";

	const rowSearchPlaceholder = activeTab === "brands" ? "Search brands…" : activeTab === "grades" ? "Search grades…" : "Search attributes…";
	...
	return (
		<>
					...
					<div className="flex min-h-0 min-w-0 flex-1 flex-col">
						{viewMode === "cards" ? (
							<div className="min-h-0 flex-1 overflow-y-auto p-2">
								<Categories embedded initialCategories={categories} initialBrands={brands} initialGrades={grades} initialAttributes={attributes} visibleSections={[]} />
							</div>
						) : (
			...
			{drawer?.kind === "grade" && <GradeEditor isOpen onClose={closeDrawerUrl} category={drawer.category} grade={drawer.grade} onSaved={refreshAll} />}
