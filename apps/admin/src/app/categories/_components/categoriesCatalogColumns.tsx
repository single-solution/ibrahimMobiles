...
export function buildGradeColumns({ selectedCategory, openDrawerUrl, openDeleteUrl }: CatalogColumnHandlers): TableColumn<AdminGrade>[] {
	return [
		{
			id: "grade",
			header: "Grade",
			...
			cell: (row) => (
				<div className="flex items-center gap-2">
					<ColoredPill backgroundColor={row.color} className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold">
						{row.label}
					</ColoredPill>
					<span className="truncate text-[10px] text-[var(--color-ink-500)]">{row.slug}</span>
				</div>
			),
		},
		{
			...
			header: "Notes",
			hideOnMobile: true,
			cell: (row) => <span className="line-clamp-2 text-[11px] text-[var(--color-ink-600)]">{row.notes || "—"}</span>,
		},
		{
			id: "status",
			header: "Live",
			hideOnMobile: true,
			cell: (row) => <CatalogVisibilityToggle endpoint={`/api/grades/${row.id}`} label={row.label} isActive={row.isActive} />,
		},
		{
			id: "actions",
			header: "",
			cell: (row) =>
				selectedCategory ? (
					<RowActions
						onEdit={() => openDrawerUrl({ kind: "grade", category: selectedCategory, grade: row })}
						onDelete={() =>
							openDeleteUrl({
								kind: "grade",
								id: row.id,
