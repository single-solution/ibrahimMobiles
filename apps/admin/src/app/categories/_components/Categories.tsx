...
/**
 * Categories workspace — responsive grid of category cards with inline brands, grades, and attributes.
 */
...
function getWorkspaceCopy(visibleSections: CatalogSection[]) {
	...
	if (visibleSections.length === 1 && visibleSections[0] === "grades") {
		return {
			...
			description: "Manage condition grades under their categories. Brand and attribute rows are hidden here.",
		};
	}
	if (visibleSections.length === 1 && visibleSections[0] === "attributes") {
		return {
			countLabel: (count: number) => `${count} attribute workspaces`,
			description: "Manage attributes under their categories. Brand and grade rows are hidden here.",
		};
	}
	return {
		countLabel: (count: number) => `${count} categories`,
		description: "Each category owns its brands, grades, and attributes. Authoring here propagates straight to the storefront.",
	};
}
...
export function Categories({
	...
	const initialGradesKey = useMemo(() => catalogEntityKey(initialGrades), [initialGrades]);
	...
	const [gradesSeedKey, setGradesSeedKey] = useState(initialGradesKey);
	if (gradesSeedKey !== initialGradesKey) {
		setGradesSeedKey(initialGradesKey);
	...
	const grouped = useMemo(() => {
		return categories.map((category) => ({
			...
			grades: grades.filter((grade) => grade.categorySlug === category.slug).sort((left, right) => left.label.localeCompare(right.label)),
	...
	async function handleConfirmDelete() {
		...
		try {
			if (kind === "brand" && unlinkFromCategorySlug) {
				...
				const path = kind === "category" ? `/api/categories/${id}` : kind === "brand" ? `/api/brands/${id}` : kind === "grade" ? `/api/grades/${id}` : `/api/attributes/${id}`;
	...
	return (
		<>
			...
			{grouped.length === 0 ? (
				...
				>
					{grouped.map(({ category, brands: brandsForCategory, grades: gradesForCategory, attributes: attrsForCategory }, index) => (
						<li key={category.id} className="reveal h-full">
							<CategoryCard
								...
								onAddGrade={() => setDrawer({ kind: "grade", category, grade: null })}
								onEditGrade={(grade) => setDrawer({ kind: "grade", category, grade })}
								onDeleteGrade={(grade) =>
									setDeleteIntent({
										kind: "grade",
										id: grade.id,
			...
			{drawer?.kind === "grade" && <GradeEditor isOpen onClose={() => setDrawer(null)} category={drawer.category} grade={drawer.grade} onSaved={refreshAll} />}
						...
						<ul className="flex flex-col gap-1.5">
							{grades.map((grade) => (
								<li key={grade.id}>
									...
									>
										<span className="inline-block size-3 shrink-0 rounded-full" style={{ backgroundColor: grade.color }} aria-hidden />
										<span className="flex-1 truncate text-[13px] font-semibold text-[var(--color-ink-900)]">{grade.label || "Untitled grade"}</span>
										<span className="truncate text-[11.5px] text-[var(--color-ink-500)]">{(grade.notes ?? "").slice(0, 60) || "No notes yet"}</span>
										<span
											role="presentation"
											onClick={(e) => {
												e.stopPropagation();
