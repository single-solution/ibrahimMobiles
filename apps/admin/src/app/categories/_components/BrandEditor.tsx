...
export function BrandEditor({
  ...
  const [form, setForm] = useState<FormState>(() =>
    brand ? formFromBrand(brand) : emptyForm(),
  );
  ...
  const draft: BrandDraft = useMemo(
    () => ({ name: deferredForm.name, isActive: deferredForm.isActive }),
    [deferredForm],
  );
  const siblingNames = useMemo(
    () =>
      siblings
        .filter((sibling) => !brand || sibling.id !== brand.id)
        ...
        <PreviewPanel
          tiles={[
            ...
            },
            {
              ...
              body: (
                <BrandFilterRowPreview brand={draft} siblingNames={siblingNames} />
              ),
            },
          ]}
        />
