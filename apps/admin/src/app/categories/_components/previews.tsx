...
export function GradeShowcasePreview({ grade }: { grade: GradeDraft }) {
	return (
		<div className="p-3">
			...
			>
				<p className="text-[11.5px] font-semibold uppercase tracking-[0.14em]" style={{ color: grade.color }}>
					{grade.label || "Grade"}
				</p>
				<StructuredContentFullPreview
				...
				/>
				{grade.video ? (
					<p className="mt-2 text-[10.5px] text-[var(--color-ink-500)]">Inspection video attached</p>
				) : (
					<p className="mt-2 text-[10.5px] italic text-[var(--color-rose-700)]">No inspection video yet</p>
				)}
			</div>
		</div>
	);
