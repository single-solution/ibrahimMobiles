...
export function OrderEditModal({
  ...
  const subtotal = editedItems.reduce(
    (acc, item) => acc + item.unitPriceRupees * item.quantity,
    0
  );
  const total = Math.max(
    0,
    subtotal + order.totals.shippingRupees - order.totals.discountRupees
  );
  ...
  return (
    ...
    >
      <form id="order-edit-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section>
          <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-ink-500)]">
            Delivery address
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            <TextField
              ...
              onChange={(e) =>
                setEditedAddress({ ...editedAddress, recipientName: e.target.value } as any)
              }
            />
            <TextField
              label="Phone"
              ...
              onChange={(e) =>
                setEditedAddress({ ...editedAddress, phoneNumber: e.target.value } as any)
              }
            />
            <div className="sm:col-span-2">
              <TextField
                ...
                onChange={(e) =>
                  setEditedAddress({ ...editedAddress, street: e.target.value } as any)
                }
              />
            </div>
          </div>
        ...
        <section>
          ...
          <ul className="space-y-3">
            ...
            {editedItems.length === 0 && (
              <p className="text-sm text-[var(--color-ink-500)] py-2">
                No items in order.
              </p>
            )}
            <div className="pt-2">
