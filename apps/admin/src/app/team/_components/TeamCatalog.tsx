...
function TeamCatalogInner({
  ...
  const filteredMembers = useMemo(() => {
    ...
    return rows.filter((row) =>
      `${row.name} ${row.email} ${row.phoneNumber ?? ""} ${ROLE_LABEL[row.role]}`
        .toLowerCase()
        .includes(query),
    );
  ...
  useEffect(() => {
    scheduleStateUpdate(() => {
      ...
      const stillVisible =
        activeId !== null && filteredMembers.some((row) => row.id === activeId);
  ...
  return (
    <>
      <WorkspaceFrame>
        {!canUpdate ? (
          <WorkspaceReadOnlyBanner message="Read-only — you can view the team but not edit roles, profiles, or remove members." />
        ) : null}
              ...
              <ul className="flex flex-col gap-0.5">
                ...
                {ROLE_ORDER.map((role) => (
                  <WorkspaceSidebarNavItem
                    ...
                    count={segmentCounts[role]}
                    isActive={segment === role}
      ...
      <ConfirmDialog
        ...
        message={
          <>
            <strong>{toRemove?.name}</strong> will lose access to the admin console immediately
            and all their open sessions are dropped.
          </>
        }
        tone="danger"
...
function TeamListItem({
  ...
  return (
    <div
      className={classNames(
        "border-b border-[var(--color-ink-100)] transition-colors",
          ...
          <span className="flex items-start justify-between gap-2">
            <span className="min-w-0">
              <span className="block truncate text-sm font-semibold text-[var(--color-ink-900)]">
                {member.name}
                {isSelf ? (
                  <span className="ml-1 text-[10px] font-medium text-[var(--color-ink-400)]">
                    (you)
                  </span>
                ) : null}
              </span>
              <span className="mt-0.5 block truncate text-[11px] text-[var(--color-ink-500)]">
                {member.email}
              </span>
            </span>
            {member.lastSignInAt ? (
