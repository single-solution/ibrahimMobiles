/**
 * Chat thread polling utilities (304 / If-None-Match).
 */
...
export function isThreadUnchangedForPoll(args: {
  ...
  if (args.since) {
    ...
    return (
      toMillis(args.lastMessageAt) <= sinceMillis &&
      toMillis(args.updatedAt) <= sinceMillis
    );
  }
  return false;
}
