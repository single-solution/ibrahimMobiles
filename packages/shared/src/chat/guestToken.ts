 ...
 *   - `phoneNumber`: snapshot at token-issue time, used for the eventual
 *     "claim history" merge once the visitor signs in (T8.10).
 *
...
function getKey(): Uint8Array {
  ...
  if (!secret) {
    throw new Error(
      "AUTH_SECRET is required to sign chat guest tokens. Set it in your environment.",
    );
  }
...
export async function verifyGuestToken(
  ...
  try {
    ...
    const inquiryIds = (payload.inquiryIds as unknown[]).filter(
...
export async function appendInquiryToGuestToken(
  ...
  const inquiryIds = existing
    ? Array.from(new Set([...existing.inquiryIds, newInquiryId]))
    : [newInquiryId];
