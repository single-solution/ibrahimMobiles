...
export function nextInquiryStatusAfterCustomerMessage(
  current: InquiryThreadStatus,
): InquiryThreadStatus | null {
  if (current === "resolved") {
    return "open";
  }
  return null;
...
export function nextInquiryStatusAfterTeamMessage(
  current: InquiryThreadStatus,
): InquiryThreadStatus | null {
  if (current === "open") {
...
export function inquiryStatusPatchAfterMessage(
  ...
  const next =
    author === "customer"
      ? nextInquiryStatusAfterCustomerMessage(current)
      : nextInquiryStatusAfterTeamMessage(current);
