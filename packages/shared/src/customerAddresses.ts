 ...
 */
export function validateCustomerAddresses(
  list: unknown,
): { addresses: ValidatedCustomerAddress[] } | { error: string } {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    const street = validateOptionalField(
      entry.street,
      `${positionLabel} street`,
      FIELD_LIMITS.addressStreet,
    );
    if (street.error) {
    ...
    const area = validateOptionalField(
      entry.area,
      `${positionLabel} area`,
      FIELD_LIMITS.addressArea,
    );
    if (area.error) {
      return { error: area.error };
    }

    const postal = validateOptionalField(
      entry.postalCode,
      `${positionLabel} postcode`,
    ...
    const labelInput = validateOptionalField(
      entry.label,
      `${positionLabel} label`,
      FIELD_LIMITS.addressLabel,
    );
    if (labelInput.error) {
