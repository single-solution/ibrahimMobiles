 ...
 */
export function compactAttributeOptionValue(
  label: string,
  unit = "",
...
export function sortAttributeOptions<T extends { label: string }>(
  options: T[],
  unit?: string,
): T[] {
  return [...options].sort((left, right) =>
    compareAlphabetically(
      formatAttributeOptionLabel(left.label, unit),
      formatAttributeOptionLabel(right.label, unit),
    ),
  );
}
...
export function resolveVariantAttributeLabel(
  attribute: AttributeLabelSource,
  value: string,
...
export function formatVariantAttributeSummary(
  ...
  for (const definition of definitions) {
    ...
    const labels = values
      ...
      .map((value) =>
        resolveVariantAttributeLabel(definition, value, attributeDisplay),
      );
    if (labels.length > 0) {
