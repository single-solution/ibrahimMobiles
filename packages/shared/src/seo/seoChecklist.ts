...
export function evaluateSeoChecklist(
  ...
  const items: SeoChecklistItem[] = [
    ...
    },
    {
      ...
      status: keyword
        ? keywordIn(resolved.title, keyword)
          ? "pass"
          : "fail"
        : "na",
    },
    {
      ...
      status: keyword
        ? keywordIn(resolved.description, keyword)
          ? "pass"
          : "fail"
        : "na",
    },
  ...
  if (entityType === "product") {
    items.push({
      ...
      status:
        entity.allVariantImagesHaveAlt === undefined
          ? "na"
          : entity.allVariantImagesHaveAlt
            ? "pass"
            : "fail",
    });
  }
  ...
  const score =
    scored.length === 0
      ? 0
      : Math.round(((passCount + warnCount * 0.5) / scored.length) * 100);
